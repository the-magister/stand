...
void loop() {
  // check sensors for activity level
  static int rampDownInterval = 10; // seconds
  byte activityLevel = activity(rampDownInterval);

  // dim stuff
  fade( activityLevel );

  // run animations on the background.
  background( activityLevel );

  // run animations in the foregroupd.
  foreground( activityLevel );

  // run any debugging code
  debug( false );
...
byte activity(int downInterval) {
  // track activity
  static byte activity;

  // anything from the PIR?
  boolean PIR = motionSensor();

  // do something with PIR
  if ( PIR ) {
    activity = 255;
    Serial << F("Activity. +PIR. activity=") << activity << endl;
  } else {
    EVERY_N_SECONDS( downInterval ) {
      activity = sub8(activity, 1);
      Serial << F("Activity. -activity=") << activity << endl;
    }
  }
...
boolean motionSensor() {
  const boolean triggerState = LOW;
  static boolean lastState = triggerState;

  // emulate the PIR.  pull the pin low every N seconds
  boolean state = !triggerState;
  EVERY_N_SECONDS( 10 ) {
    state = triggerState;
  }
  ...
  if ( state != lastState ) {
    ...
    if ( state == triggerState ) {
      Serial << F("Motion. triggered at ") << millis() << endl;
      return ( true );
    }
  }
...
void fade(byte activityLevel) {
  // fade amount each update
  static byte fadeEverything = 16;

  leds.fadeToBlackBy(fadeEverything);
}

void background(byte activityLevel) {
#define RUN_TOP 0
#define RUN_LEGS 0
#define RUN_OVERHEAD_RAINBOW 1
#define RUN_EXTENT 0

  byte fadeBy = map(activityLevel, 0, 255, 255-1, 196);
  ...
  if ( RUN_TOP ) {
    ...
    for ( byte i = 0; i < NUM_CONTROLLERS; i++) {
      CTOP[i][topLoc] += CHSV(topHue, 255, activityLevel); // paint
...
void foreground(byte activityLevel) {
...
#define RUN_OVERHEAD_SPOTS 1
  ...
  if ( RUN_TRACK ) {
    ...
    CHSV trackColor = CHSV(HUE_BLUE, 0, activityLevel); // color choice.  white.
  ...
  if ( RUN_LOOPS ) {
    CHSV lTrackColor = CHSV(HUE_BLUE, 0, activityLevel); // color choice.  white.
    CHSV rTrackColor = CHSV(HUE_BLUE, 0, activityLevel); // color choice.  white.
    CHSV tTrackColor = CHSV(HUE_BLUE, 0, activityLevel); // color choice.  white.
  ...
  if ( RUN_FIRE ) {
    byte cooling = map(activityLevel, 0, 255, 250, 196);
    byte sparks = map(activityLevel, 0, 255, 5, 20);
    Fire2012WithPalette( cooling, sparks );
  }

  if ( RUN_LEG_JUGGLE ) {
    byte dots = map(activityLevel, 0, 255, 1, 5);
    JuggleLegs( dots );
  }
